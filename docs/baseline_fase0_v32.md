...
> **2026-09-06 — actualizado a los números vigentes** (106 files / 4745 tests,
...
| Tests | **4745** (4743 passed, 2 skipped, 0 failed) |
| Duración | ~16 s |
...
- ✅ **Job `vitest` + lint** en `.github/workflows/webui-ci.yml` (ubuntu-latest): suite
  completa de WebUI (**106 files / 4745 tests, 0 fallos**) y ESLint **0 errores / 0
...
- ✅ **Job `registry-generation`** en `.github/workflows/registry-generation.yml`
  ...
  - **Verificado**: regeneración → exit 0, 0 diffs de contenido (solo timestamp).
    Registro: 236 parámetros (226 físicos · 3 extendidos · 7 virtuales).
...
0. **Fase 0 — Inventario, Baseline y Perfilado (Pre-requisito):** este documento es la
   baseline exacta exigida por el plan — suites (106 files / 4745 tests WebUI + 126 suites /
...
2. **Fase 1 — Esquema Declarativo, Generador y Pre-validación:** `schemas/parameter-registry.json`
   ...
   `schemas/parameter-registry.data.json`, 236 parámetros). Jobs CI dedicados: `schema-validation`
