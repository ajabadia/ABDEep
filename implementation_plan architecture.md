...
> - **Suites:** WebUI 106 files / 4745 tests (0 fallos, 2 skipped) + guard
...
> Vitest (2026-09-06): 106 files / 4745 tests / 0 fallos.
...
- [x] Job `hw-dump-validate` (`.github/workflows/hardware-dump-validate.yml`): validación OFFLINE (sin hardware, ubuntu-latest) de los dumps commiteados del DM12 (`resources/hardware_dumps/`) contra su `manifest.json` y el corpus de fábrica vía `node scripts/hw_bank_dump.js --validate-committed --check-payloads` — tamaño canónico 37248 B, SHA-256 == `rawSha256` del manifest y diffs de payload vs corpus == `payloadDiffPrograms` registrados (incluye la divergencia conocida B/1 → known_exception). Falla si manifest, dumps o corpus divergen en un PR.
...
- 🔎 **Verificación documental (`.github/workflows/docs-verification.yml`):** el job CI `docs-verification` comprueba que los 13 jobs de Fase 7 listados aquí coinciden con `docs/baseline_fase0_v32.md` §7 (mismo contrato) y que cada job tiene su workflow real — falla ante cualquier divergencia entre plan y doc.
