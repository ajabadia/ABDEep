...
function initArpeggiatorModal() {
    ...
    if (selectClock) {
        selectClock.addEventListener('change', () => {
            ...
            var _clockNames_ = ['1/1','1/2','1/3','1/4','1/6','1/8','1/12','1/16','1/24','1/32','1/48','1/64','1/96'];
            var _clockIdx_ = parseInt(selectClock.value);
    ...
    if (selectVelGate) {
        selectVelGate.addEventListener('change', () => {
            ...
            var _vgNames_ = ['Gate','Velocity','Seq'];
            var _vgIdx_ = parseInt(selectVelGate.value);
            var _lcd_ = document.getElementById('lcd-text');
            if (_lcd_) {
    ...
    if (selectMode) {
        selectMode.addEventListener('change', () => {
            ...
            var _modeNames_ = ['UP','DOWN','UP-DOWN','UP-INV','DOWN-INV','UP-DN-INV','UP-ALT','DOWN-ALT','RANDOM','AS-PLAYED'];
            var _modeIdx_ = parseInt(selectMode.value);
            var _lcd_ = document.getElementById('lcd-text');
            if (_lcd_) {
    ...
    if (selectOctave) {
        selectOctave.addEventListener('change', () => {
            ...
            var _octVal_ = parseInt(selectOctave.value) + 1;
            var _lcd_ = document.getElementById('lcd-text');
            if (_lcd_) {
    ...
    function _loadUserArpPresets() {
        try {
            var raw = localStorage.getItem('abd-eep-arp-presets');
            if (raw) {
    ...
    var resetBtn = document.getElementById('modal-arp-reset-btn');
    if (resetBtn) {
        resetBtn.addEventListener('click', function() {
            ...
            var _aNotes_ = bridge._arpEngine.heldNotes.length;
            var _aStep_ = bridge._arpEngine.stepIndex;
            var _aBar_ = window._genFillBar(Math.round((_aNotes_ / 12) * 18), 18);
            var _lcdText_ = document.getElementById('lcd-text');
            if (_lcdText_) {
                var _arpHtml_ = window._genLcdBarHtml('arp', {
        ...
        window.dualMidiBridge.onParameterChanged((paramId, val) => {
            ...
            if (paramId === "arp_enable" && arpBox) {
                ...
                if (val > 0.5 && window.dualMidiBridge._arpEngine) {
                    var _aNotes_ = window.dualMidiBridge._arpEngine.heldNotes.length;
                    var _aStep_ = window.dualMidiBridge._arpEngine.stepIndex;
                    window._arpLastResetTime = Date.now();
                    window._arpResetCount++;
                    var _aBar_ = window._genFillBar(Math.round((_aNotes_ / 12) * 18), 18);
                    var _lcdText_ = document.getElementById('lcd-text');
