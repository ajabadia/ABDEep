            ...
            if (queueMsg && queueMsg.priority < 2 && !lcdText._lcdFading) {
                ...
                if (hasPb || hasAt || hasMw) {
                    ...
                    if (window._seqDebugMode) {
                        let seqDebugHtml = '<span style="font-size:9px; opacity:0.6;">SEQ DEBUG <span style="color:var(--color-danger);">●</span></span><br>';
                        ...
                        for (let di = 0; di < _debugSeqLen_; di++) {
                            ...
                            const isSkip = raw === 0;
                            const valStr = isSkip ? '<span style="color:var(--color-danger)">SKIP</span>' : (bip >= 0 ? '+' + bip : '' + bip);
                            const color = isSkip ? 'var(--color-danger)' : (bip > 0 ? 'var(--accent-pink)' : (bip < 0 ? 'var(--text-dim)' : 'var(--text-faint)'));
                    ...
                    const basePatchName_ = activePatch ? activePatch.name : (bridge && bridge.parameterCache && bridge.parameterCache['patch_name'] ? bridge.parameterCache['patch_name'] : 'INITIAL PATCH');
                    const baseBankName_ = window.currentActiveBank || '';
                    const debugBadge_ = window._seqDebugMode ? '<span style="float:right;font-size:6px;color:var(--color-danger);border:1px solid var(--color-danger);padding:0 2px;border-radius:1px;line-height:10px;">DBG</span>' : '';
                    ...
                    if (window._twText) {
                        ...
                        const charsToShow_ = Math.min(window._twText.length, Math.max(1, Math.floor(elapsed_ / _TW_CHAR_MS)));
                        const revealed_ = window._twText.substring(0, charsToShow_);
                        const isDone_ = charsToShow_ >= window._twText.length;
                        ...
                        if (basePatchName_.length > 10) {
                            ...
                            _pnHtml_ = '<div class="lcd-scroll-container"><span class="lcd-scroll-text animate" style="font-size:16px;color:var(--text-primary);letter-spacing:1px;animation-duration:' + _scrollDur_.toFixed(1) + 's;">' + _pnUpper_ + '</span></div>';
                        } else {
                            _pnHtml_ = '<span style="font-size:16px;color:var(--text-primary);letter-spacing:1px;">' + _pnUpper_ + '</span>';
                        }
                        lcdText.innerHTML = _pnHtml_ + debugBadge_ + '<br><span style="font-size:7px;color:var(--text-faint);letter-spacing:0.5px;">' + baseBankName_.toUpperCase() + '</span>';
                    }
                }
            }
        }

        const _glowEl_ = document.getElementById('lcd-glow-pulse');
        if (_glowEl_) {
            const _gSeqEn_ = bridge.parameterCache && (bridge.parameterCache['seq_enable'] || 0) > 0.5;
            const _gStep_ = bridge.parameterCache ? bridge.parameterCache['seq_current_step'] : undefined;
            const _gStepChanged_ = _gStep_ !== undefined && _gStep_ !== _seqGlowLastStep;
            if (_gStepChanged_) {
                _seqGlowLastStep = _gStep_;
                _seqGlowPulse = Date.now();
            }
            if (_gSeqEn_ && _gStep_ !== undefined) {
                const _gAge_ = Date.now() - _seqGlowPulse;
                const _gDur_ = 400;
                if (_gAge_ < _gDur_) {
                    const _gInt_ = 1.0 - (_gAge_ / _gDur_);
                    const _gSize_ = 2 + _gInt_ * 4;
