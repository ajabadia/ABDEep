...
function fxTypeNameFromNormalized(normalizedType) {
    const idx = fxTypeIndexFromNormalized(normalizedType);
...
function validatePresetEntry(preset) {
    ...
    if (!Array.isArray(preset.patches)) {
        ...
        for (let j = 0; j < preset.patches.length; j++) {
...
function groupPresetsByType(presets) {
    ...
    for (let i = 0; i < presets.length; i++) {
        const typeName = fxTypeNameFromNormalized(presets[i].type);
...
describe('validatePresetEntry — structural validation', function() {
    it('validates a correct preset entry', function() {
        ...
        const result = validatePresetEntry(preset);
    ...
    it('rejects missing name', function() {
        const result = validatePresetEntry({ type: 0.5, gain: 0.5, params: new Array(12).fill(0), patches: ['A-001'] });
        expect(result.valid).toBe(false);
        expect(result.errors.some(function(e) { return e.indexOf('name') !== -1; })).toBe(true);
    });

    it('rejects empty name', function() {
        const result = validatePresetEntry({ name: '', type: 0.5, gain: 0.5, params: new Array(12).fill(0), patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects type out of [0,1]', function() {
        const result = validatePresetEntry({ name: 'T', type: 1.5, gain: 0.5, params: new Array(12).fill(0.5), patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects gain out of [0,1]', function() {
        const result = validatePresetEntry({ name: 'T', type: 0.5, gain: -0.1, params: new Array(12).fill(0.5), patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects NaN type', function() {
        const result = validatePresetEntry({ name: 'T', type: NaN, gain: 0.5, params: new Array(12).fill(0.5), patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects params that are not an array', function() {
        const result = validatePresetEntry({ name: 'T', type: 0.5, gain: 0.5, params: 'not array', patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects params with wrong length', function() {
        const result = validatePresetEntry({ name: 'T', type: 0.5, gain: 0.5, params: [0.1], patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects params with out-of-range value', function() {
        const params = new Array(12).fill(0.5);
        params[5] = 1.5;
        const result = validatePresetEntry({ name: 'T', type: 0.5, gain: 0.5, params: params, patches: ['A-001'] });
        expect(result.valid).toBe(false);
    });

    it('rejects empty patches', function() {
        const result = validatePresetEntry({ name: 'T', type: 0.5, gain: 0.5, params: new Array(12).fill(0.5), patches: [] });
        expect(result.valid).toBe(false);
    });

    it('rejects patches with non-string entries', function() {
        const result = validatePresetEntry({ name: 'T', type: 0.5, gain: 0.5, params: new Array(12).fill(0.5), patches: [42] });
        expect(result.valid).toBe(false);
    });
});
...
describe('SAMPLE_PRESETS — each entry is valid', function() {
    for (let i = 0; i < SAMPLE_PRESETS.length; i++) {
        (function(idx, preset) {
...
describe('SAMPLE_PRESETS — value ranges', function() {
    ...
    it('all param values are in [0, 1]', function() {
        for (let i = 0; i < SAMPLE_PRESETS.length; i++) {
            for (let j = 0; j < SAMPLE_PRESETS[i].params.length; j++) {
                const p = SAMPLE_PRESETS[i].params[j];
    ...
    it('all patches entries are strings', function() {
        for (let i = 0; i < SAMPLE_PRESETS.length; i++) {
            for (let j = 0; j < SAMPLE_PRESETS[i].patches.length; j++) {
...
describe('SAMPLE_PRESETS — type mapping resolves correctly', function() {
    ...
    it('each preset maps to correct type name', function() {
        for (let i = 0; i < SAMPLE_PRESETS.length; i++) {
            ...
            for (const prefix in TYPE_PREFIX_MAP) {
            ...
            if (expectedType) {
                const actualType = fxTypeNameFromNormalized(preset.type);
    ...
    it('no two presets share the exact same name', function() {
        const dupResult = findDuplicateNames(SAMPLE_PRESETS);
...
describe('SAMPLE_PRESETS — type quantization', function() {
    it('all type values round to known indices (0-35)', function() {
        for (let i = 0; i < SAMPLE_PRESETS.length; i++) {
            const idx = fxTypeIndexFromNormalized(SAMPLE_PRESETS[i].type);
    ...
    it('type values are exact multiples of 1/35 (within 5 decimal precision)', function() {
        for (let i = 0; i < SAMPLE_PRESETS.length; i++) {
            const t = SAMPLE_PRESETS[i].type;
            const idx = fxTypeIndexFromNormalized(t);
            const expectedPrecise = idx / 35.0;
...
describe('groupPresetsByType — grouping', function() {
    const groups = groupPresetsByType(SAMPLE_PRESETS);
