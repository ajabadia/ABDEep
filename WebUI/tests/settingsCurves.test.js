...
function hitTest(cvX, cvY, points, bipolar, canvasWidth, canvasHeight, pad, threshold) {
    ...
    for (let i = 0; i < points.length; i++) {
        const px = curveToCanvasX(points[i].x, canvasWidth, pad);
        const py = curveToCanvasY(points[i].y, bipolar, canvasHeight, pad);
...
function addCustomPoint(cvX, cvY, points, bipolar, canvasWidth, canvasHeight, pad) {
    pad = pad || 8;
    const nx = Math.max(0.01, Math.min(0.99, canvasXToCurve(cvX, canvasWidth, pad)));
    let ny = Math.max(0, Math.min(1, canvasYToCurve(cvY, bipolar, canvasHeight, pad)));
    if (bipolar) {ny = Math.max(-1, Math.min(1, ny));}

    const intermediates = points.filter(function(p) { return p.x > 0.001 && p.x < 0.999; });
    if (intermediates.length >= 5) {return null;}

    const newPoint = {x: nx, y: ny};
    const result = [].concat(intermediates);
...
function getCurveLabel(curveType, bipolar) {
    const name = CURVE_LABELS[curveType] || curveType;
...
describe('canvasXToCurve — pixel to normalized coordinate', function() {
    const canvasW = 256, pad = 8;
    const graphW = 256 - 16; // 240
...
describe('curveToCanvasX — normalized to pixel coordinate', function() {
    ...
    it('round-trip: curveToCanvasX ∘ canvasXToCurve', function() {
        const x = 0.3;
        const cvX = curveToCanvasX(x, canvasW, pad);
        const back = canvasXToCurve(cvX, canvasW, pad);
        expect(back).toBeCloseTo(x, 5);
    });
});

describe('unipolar canvasYToCurve and curveToCanvasY', function() {
    const canvasH = 256, pad = 8;
    ...
    it('round-trip unipolar', function() {
        const y = 0.3;
        const cvY = curveToCanvasY(y, false, canvasH, pad);
        const back = canvasYToCurve(cvY, false, canvasH, pad);
...
describe('bipolar coordinate conversion', function() {
    const canvasH = 256, pad = 8;
    const graphH = canvasH - pad * 2; // 240
    const centerY = pad + graphH / 2; // 128
    ...
    it('round-trip bipolar', function() {
        const y = -0.3;
        const cvY = curveToCanvasY(y, true, canvasH, pad);
        const back = canvasYToCurve(cvY, true, canvasH, pad);
        expect(back).toBeCloseTo(y, 5);
    });
...
describe('hitTest — point detection in canvas', function() {
    let points, canvasW, canvasH, pad;
    ...
    it('returns index 0 when clicking near first point', function() {
        ...
        const px = curveToCanvasX(points[0].x, canvasW, pad);
        const py = curveToCanvasY(points[0].y, false, canvasH, pad);
        expect(hitTest(px, py, points, false, canvasW, canvasH, pad)).toBe(0);
    });

    it('returns index 2 when clicking near third point', function() {
        const px = curveToCanvasX(points[2].x, canvasW, pad);
        const py = curveToCanvasY(points[2].y, false, canvasH, pad);
    ...
    it('near miss is still a miss (slightly outside threshold)', function() {
        ...
        const px = curveToCanvasX(points[1].x, canvasW, pad);
        const py = curveToCanvasY(points[1].y, false, canvasH, pad);
...
describe('addCustomPoint — adding intermediate points', function() {
    ...
    it('adds intermediate point (source stores only intermediates, excludes endpoints)', function() {
        ...
        const cvX = curveToCanvasX(0.3, 256, 8);
        const cvY = curveToCanvasY(0.2, false, 256, 8);
        const result = addCustomPoint(cvX, cvY, points, false, 256, 256, 8);
    ...
    it('clamps x to [0.01, 0.99] (not on endpoints)', function() {
        const cvX = curveToCanvasX(0.0, 256, 8); // maps to 0
        const cvY = curveToCanvasY(0.5, false, 256, 8);
        const result = addCustomPoint(cvX, cvY, points, false, 256, 256, 8);
    ...
    it('returns null when there are already 5 intermediate points', function() {
        const manyPoints = [{x:0,y:0},{x:0.1,y:0.1},{x:0.2,y:0.2},{x:0.3,y:0.3},{x:0.4,y:0.4},{x:0.5,y:0.5},{x:1,y:1}];
        // That's 5 intermediates (indices 1-5)
        const cvX = curveToCanvasX(0.6, 256, 8);
        const cvY = curveToCanvasY(0.6, false, 256, 8);
        const result = addCustomPoint(cvX, cvY, manyPoints, false, 256, 256, 8);
        expect(result).toBeNull();
    });

    it('handles bipolar y clamping', function() {
        // In bipolar mode, y is clamped to [-1, 1]
        const cvY = curveToCanvasY(2.0, true, 256, 8); // y=2 is outside canvas visually
        // cvY = centerY - 2 * graphH/2 = 128 - 240 = -112
        // canvasYToCurve(-112, true, 256, 8) = (128 - (-112)) / 120 = 240/120 = 2
        // So ny = Math.max(-1, Math.min(1, 2)) = 1
        const cvX = curveToCanvasX(0.5, 256, 8);
        const result = addCustomPoint(cvX, cvY, points, true, 256, 256, 8);
...
describe('deleteCustomPoint — removing intermediate points', function() {
    ...
    it('removes an intermediate point (not endpoint)', function() {
        const result = deleteCustomPoint(1, points); // Remove middle point (index 1 in full, index 0 in intermediates)
    ...
    it('returns empty array when only one intermediate and it is deleted', function() {
        const twoPoints = [{x:0,y:0},{x:0.5,y:0.5},{x:1,y:1}];
        const result = deleteCustomPoint(1, twoPoints);
...
describe('_evalCustomCurve integration (core curve math)', function() {
    it('interpolates with custom points from curve editor', function() {
        ...
        const points = [{x:0,y:0}, {x:0.3,y:0.1}, {x:0.7,y:0.9}, {x:1,y:1}];
