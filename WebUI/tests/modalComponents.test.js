...
describe('ArpModal (arp-modal.js)', () => {
  ...
  it('has 5 select elements (clock, velgate, mode, pattern, octave)', () => {
    expect(countSelects(getArpModalTemplate())).toBe(5);
  });

  it('clock select has 13 options (1/2 to 1/48)', () => {
    const html = getArpModalTemplate();
  ...
  it('pattern select has 65 options (None + 32 Preset + 32 User)', () => {
    const html = getArpModalTemplate();
    const patternOpts = html.match(/id="modal-arp-pattern-select"[^>]*>([\s\S]*?)<\/select>/);
    expect(patternOpts).not.toBeNull();
    const opts = patternOpts[1].match(/<option/g);
    expect(opts ? opts.length : 0).toBe(65);
    expect(html).toContain('None');
    expect(html).toContain('Preset 1');
    expect(html).toContain('Preset 32');
    expect(html).toContain('User 1');
    expect(html).toContain('User 32');
  });
